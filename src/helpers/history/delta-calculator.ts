...
import { deriveHistorySubjectHints } from './presentation';
...
export function calculateDelta(
	...
	const delta = { operation, entityType, changes };

	return {
		...delta,
		subjectHints: deriveHistorySubjectHints(delta, {
			nodes: newState.nodes,
			edges: newState.edges,
			previousNodes: oldState.nodes,
			previousEdges: oldState.edges,
		}),
	};
}
	...
	for (const ch of changes) {
		...
		if (change.type === 'node') {
			if (change.op === 'remove') {
				...
				const idx = nodes.findIndex((n) => n.id === change.id);
				if (idx !== -1)
					nodes[idx] = applyDottedPatch(nodes[idx], change.patch) as AppNode;
			}
			...
			if (change.op === 'remove') {
				...
				const idx = edges.findIndex((e) => e.id === change.id);
				if (idx !== -1)
					edges[idx] = applyDottedPatch(edges[idx], change.patch) as AppEdge;
			}
		}
	}
...
function toPatchOp(change: any): HistoryPatchOp {
	...
	if (before && after)
		return { id, type, op: 'patch', patch: flattenDotted(after) };
...
function applyDottedPatch<T extends object>(
	obj: T,
...
function setByPath(target: any, path: string, value: any) {
	...
	for (const part of parts) {
		if (!isSafeKey(part)) {
			console.warn(
				`Blocked unsafe property assignment: ${part} in path ${path}`
			);
			return;
		}
	}
...
function createReversePatch(
	originalObject: any,
	forwardPatch: Record<string, any>
): Record<string, any> {
