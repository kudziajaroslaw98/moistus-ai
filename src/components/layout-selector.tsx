...
function LayoutConfigPanel({ config, onConfigChange }: LayoutConfigPanelProps) {
  const updateConfig = (updates: Partial<SpecificLayoutConfig>) => {
        ...
        {config.nodeSpacing !== undefined && (
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) => updateConfig({ nodeSpacing: Number(e.target.value) })}
              className="w-full"
            />
            <span className="text-xs text-zinc-500">{config.nodeSpacing}px</span>
          </div>
        )}
        ...
        {config.rankSpacing !== undefined && (
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) => updateConfig({ rankSpacing: Number(e.target.value) })}
              className="w-full"
            />
            <span className="text-xs text-zinc-500">{config.rankSpacing}px</span>
          </div>
        )}
      </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) => updateConfig({ iterations: Number(e.target.value) } as Partial<SpecificLayoutConfig>)}
              className="w-full"
            />
            <span className="text-xs text-zinc-500">{(config as { iterations?: number }).iterations || 300}</span>
          </div>
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) => updateConfig({ strength: Number(e.target.value) } as Partial<SpecificLayoutConfig>)}
              className="w-full"
            />
            <span className="text-xs text-zinc-500">{(config as { strength?: number }).strength || -300}</span>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) => updateConfig({ radius: Number(e.target.value) } as Partial<SpecificLayoutConfig>)}
              className="w-full"
            />
            <span className="text-xs text-zinc-500">{(config as { radius?: number }).radius || 200}px</span>
          </div>
          <div className="space-y-1">
            ...
            <input
              ...
              min="0"
              max="360"
              value={((config as { startAngle?: number }).startAngle || 0) * (180 / Math.PI)}
              onChange={(e) => updateConfig({ startAngle: Number(e.target.value) * (Math.PI / 180) } as Partial<SpecificLayoutConfig>)}
              className="w-full"
            />
            <span className="text-xs text-zinc-500">{Math.round(((config as { startAngle?: number }).startAngle || 0) * (180 / Math.PI))}°</span>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) => updateConfig({ columns: Number(e.target.value) } as Partial<SpecificLayoutConfig>)}
              className="w-full"
            />
            <span className="text-xs text-zinc-500">{(config as { columns?: number }).columns || 4}</span>
          </div>
          <div className="space-y-1">
            ...
            <input
              ...
              onChange={(e) => updateConfig({ cellWidth: Number(e.target.value) } as Partial<SpecificLayoutConfig>)}
              className="w-full"
            />
            <span className="text-xs text-zinc-500">{(config as { cellWidth?: number }).cellWidth || 350}px</span>
          </div>
        </div>
      )}
...
export function LayoutSelector() {
  ...
  const [currentConfig, setCurrentConfig] = useState<SpecificLayoutConfig | null>(null);

  const { applyAdvancedLayout, getLayoutPresets, currentLayoutConfig } = useAppStore(
    useShallow((state) => ({
  ...
  const categorizedPresets = presets.reduce((acc, preset) => {
  ...
  const handleConfigChange = (config: SpecificLayoutConfig) => {
          ...
          {Object.entries(categorizedPresets).map(([category, categoryPresets]) => (
            <div key={category} className="p-2">
              <div className="flex items-center gap-2 mb-2 px-2">
                <div className={cn("size-3 rounded", layoutCategories[category as keyof typeof layoutCategories]?.color)} />
                <span className="text-xs font-medium text-zinc-400 uppercase tracking-wide">
                  {layoutCategories[category as keyof typeof layoutCategories]?.name || category}
                </span>
              </div>
              ...
              <div className="space-y-1">
                {categoryPresets.map((preset) => {
                  ...
                  const isCurrent = currentLayoutConfig?.algorithm === preset.config.algorithm;
