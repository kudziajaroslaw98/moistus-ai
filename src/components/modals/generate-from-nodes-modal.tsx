      ...
      <div className="flex flex-col gap-6">
        <div className="bg-zinc-800/40 p-4 rounded-md border border-zinc-700">
          <p className="text-sm text-zinc-300">
            Using AI to generate content based on <span className="font-semibold text-teal-400">{selectedNodeCount}</span> selected {selectedNodeCount === 1 ? "node" : "nodes"}
          </p>
          {isLoading && (
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="ai-prompt" className="text-sm font-medium text-zinc-300">
            Your prompt
          </label>
          <Textarea
            id="ai-prompt"
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-zinc-800">
          <Button variant="ghost" onClick={() => {
            onClose();
            setPrompt("");
          }} disabled={isLoading} className="text-zinc-300 hover:text-zinc-100">
            Cancel
          </Button>
          <Button 
