...
export function JoinRoom({
  ...
  const {
    ...
    defaultValues: {
      roomCode: roomCode.replace('-', '').toUpperCase(),
      displayName: '',
    },
  });
  ...
  const handleRoomCodeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValue('roomCode', formatted.replace('-', ''));
  };
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 shadow-xl">
        ...
        <div className="mt-4 p-3 bg-zinc-800/50 border border-zinc-700 rounded-md">
          <p className="text-zinc-400 text-xs text-center">
            You&apos;ll join as a guest. You can upgrade to a full account later to save your progress.
          </p>
        </div>
      </div>
    </motion.div>
