		...
		return (
			<>
				<div
					ref={containerRef}
					className={cn(
						'validation-trigger relative h-full min-h-0',
						...
						>
							...
							<div
								className={cn(
									...
									hasErrors ? 'bg-red-950/95' : 'bg-yellow-950/95'
								)}
							/>
							...
							<div className='space-y-3 pr-8'>
											...
											<div className='flex-1 min-w-0'>
												...
												{error.suggestion && !error.quickFixes && (
													<p className='text-xs opacity-80 mt-1 leading-tight'>
														...
														<code className='px-1.5 py-0.5 rounded bg-black/30 text-xs font-mono'>
															{error.suggestion}
														</code>
													</p>
												)}
										...
										{error.quickFixes &&
											...
											onQuickFix && (
												<div className='flex flex-wrap gap-2 ml-6'>
													{error.quickFixes.map(
														(fix: any, fixIndex: number) => (
															<button
																key={fixIndex}
								...
								{errors.length > 3 && (
									<div className='flex items-center gap-2 pl-6 pt-2 border-t border-white/10'>
										<span className='text-xs opacity-75 gap-1 flex'>
											<span>+</span>

											<span>{errors.length - 3}</span>

											<span>more</span>

											<span>{errors.length > 4 ? 'issues' : 'issue'}</span>
										</span>
									</div>
								)}
							</div>
