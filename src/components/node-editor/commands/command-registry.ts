...
export class CommandRegistry {
  ...
  public registerCommand(
    ...
    if (!replace && this.commands.has(command.id)) {
      console.warn(`Command with ID '${command.id}' already exists. Use replace option to override.`);
      return false;
    }
  ...
  private registerDefaultNodeTypeCommands(): void {
    ...
    nodeTypeCommands.forEach(cmdData => {
      const command: Command = {
        ...cmdData,
        id: cmdData.trigger,
        action: this.createNodeTypeSwitchAction(this.getNodeTypeFromTrigger(cmdData.trigger))
      };
   ...
   */
  private createNodeTypeSwitchAction(nodeType: AvailableNodeTypes): (context: CommandContext) => CommandResult {
   ...
   */
  private createPatternInsertionAction(trigger: string): (context: CommandContext) => CommandResult {
    return (context: CommandContext): CommandResult => {
      let insertText = '';
   ...
   */
  private createFormatAction(trigger: string): (context: CommandContext) => CommandResult {
    return (context: CommandContext): CommandResult => {
      ...
      if (selection) {
        ...
        newText = currentText.substring(0, from) + formattedText + currentText.substring(to);
   ...
   */
  private createTemplateAction(trigger: string): (context: CommandContext) => CommandResult {
    return (context: CommandContext): CommandResult => {
      let template = '';
