...
export const nodeEditorTheme = EditorView.theme({
  ...
  '.cm-tooltip-autocomplete.cm-tooltip': {
    ...
    boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.4), 0 10px 10px -5px rgba(0, 0, 0, 0.2) !important',
  ...
  'completion-section::after': {
    ...
    right: '8px',
    height: '1px',
    background: 'linear-gradient(90deg, transparent 0%, rgba(161, 161, 170, 0.25) 15%, rgba(161, 161, 170, 0.4) 50%, rgba(161, 161, 170, 0.25) 85%, transparent 100%)',
    borderRadius: '0.5px',
  },
...
/**
 * Optional: Create custom theme with overrides (for future extensibility)
 */
export function createNodeEditorTheme(overrides: Record<string, any> = {}): Extension {
  return EditorView.theme({
    // Base theme styles
    ...nodeEditorTheme.value.specs[0].style,
    // Custom overrides
    ...overrides
  }, { dark: true });
}