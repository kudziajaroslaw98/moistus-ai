...
import type { Command, CommandTrigger as CommandTriggerType } from '../commands/types';
  ...
  while ((match = PATTERNS.nodeType.exec(text)) !== null) {
    ...
    if (isAtTrigger) {
      ...
      return {
        ...
        error: !isValid ? `Unknown node type: ${nodeTypeInput}` :
               !hasRegisteredCommand ? `No command registered for: ${trigger}` : 
               undefined
      };
    }
...
export function detectSlashCommand(
  ...
  while ((match = PATTERNS.slashCommand.exec(text)) !== null) {
    ...
    if (isAtCommand) {
      ...
      const isValidPosition = beforeMatch.length === 0 || /\s$/.test(beforeMatch);
 ...
 */
function findMatchingSlashCommands(trigger: string, partialCommand: string): Command[] {
  if (!partialCommand) {
  ...
  const unique = combined.filter((cmd, index, arr) => 
...
export function getValidNodeTypeTriggers(): string[] {
  return [
    '$note', '$task', '$code', '$image', '$link', '$resource',
    '$question', '$annotation', '$text', '$default'
  ];
}
 ...
 */
export function debugParseText(text: string, cursorPos: number): {
  text: string;
  cursorPos: number;
