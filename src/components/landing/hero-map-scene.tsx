...
function SceneCard({
	...
	return (
		<motion.div
			className={`absolute rounded-2xl border border-white/12 bg-[linear-gradient(180deg,rgba(17,21,31,0.96),rgba(11,14,20,0.92))] px-4 py-3 shadow-[0_20px_60px_rgba(0,0,0,0.28)] backdrop-blur-xl ${className}`}
			initial={
				shouldReduceMotion
					? { opacity: 1, y: 0 }
...
function SceneNode({
	...
	const toneClasses =
		tone === 'coral'
			? 'border-brand-coral/35 bg-brand-coral/10 text-brand-coral'
			: tone === 'ghost'
				? 'border-dashed border-primary-400/35 bg-primary-500/8 text-primary-200'
				: 'border-primary-400/35 bg-primary-500/12 text-primary-100';

	return (
		<motion.div
			className={`absolute rounded-2xl border px-4 py-3 shadow-[0_16px_44px_rgba(0,0,0,0.26)] backdrop-blur-xl ${toneClasses} ${className}`}
			initial={
				shouldReduceMotion
					? { opacity: 1, scale: 1 }
...
export function HeroMapScene() {
	...
	return (
		<div className='relative mx-auto w-full max-w-[38rem]'>
			...
			<div className='relative aspect-[1.08] overflow-hidden rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(11,14,20,0.96),rgba(8,10,16,0.88))] p-3 shadow-[0_30px_90px_rgba(0,0,0,0.42)] md:p-4'>
				...
				<div className='absolute inset-3 rounded-[1.55rem] border border-white/6 bg-[linear-gradient(180deg,rgba(255,255,255,0.03),rgba(255,255,255,0.01))] md:inset-4' />

				<div className='absolute left-5 top-5 z-10 inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/25 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-text-secondary backdrop-blur-xl'>
				...
				{cursors.map((cursor) => (
					<motion.div
						key={cursor.label}
						className={`absolute z-20 inline-flex items-center gap-2 rounded-full border border-white/10 bg-black/35 px-2.5 py-1 text-[11px] font-medium text-white shadow-[0_8px_24px_rgba(0,0,0,0.25)] backdrop-blur-xl ${cursor.className}`}
						initial={
							shouldReduceMotion
								? { opacity: 1, scale: 1 }
				...
				<div className='absolute inset-x-4 bottom-4 z-10 grid grid-cols-3 gap-2 md:inset-x-5 md:bottom-5'>
					{legendItems.map((item, index) => {
						...
						return (
							<motion.div
								key={item.label}
								className='rounded-2xl border border-white/8 bg-black/25 px-3 py-3 text-center backdrop-blur-xl'
								initial={
									shouldReduceMotion
										? { opacity: 1, y: 0 }
										: { opacity: 0, y: 12 }
