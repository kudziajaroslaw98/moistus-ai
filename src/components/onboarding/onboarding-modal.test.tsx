...
describe('OnboardingModal mobile rendering', () => {
	...
	it('renders the mobile intro as a bottom-sheet flow', () => {
		...
		expect(
			screen.getByRole('button', { name: 'Skip walkthrough' })
		).toBeInTheDocument();
		expect(
	...
	it('renders the mobile checklist under the top bar and minimizes back into a pill', () => {
		...
		fireEvent.click(
			screen.getByRole('button', { name: /minimize walkthrough/i })
		);
	...
	it('does not render skip walkthrough on an active hint surface', async () => {
		...
		expect(
			screen.queryByRole('button', { name: 'Skip walkthrough' })
		).not.toBeInTheDocument();
	});
	...
	it('expands the minimized mobile pill back into the checklist', () => {
		...
		fireEvent.click(
			screen.getByRole('button', { name: /expand walkthrough/i })
		);
	...
	it('falls back to the checklist when the refreshed desktop controls target is unresolved, then returns to the coachmark once it resolves', async () => {
		...
		expect(screen.getByTestId('onboarding-checklist')).toBeInTheDocument();
		expect(
			screen.queryByTestId('onboarding-coachmark')
		).not.toBeInTheDocument();
	...
	it('does not render the upsell while subscription resolution is still pending', () => {
		mockIsMobile = false;
		mockState = {
			...mockState,
			onboardingStatus: 'upsell',
		...
		expect(screen.queryByTestId('onboarding-upsell')).not.toBeInTheDocument();
	});
});
