...
- **Audience-first writing**: Changelog entries should explain outcomes for normal end users (what changed for them), not deep implementation internals.
- **No duplicate day-category sections**: Before adding a `### Category` subsection for a date, check whether that same subsection already exists for that day. If it exists, append new bullets there instead of creating a second duplicate subsection.
- **Do not add `<!-- Updated: ... -->` comments inside `CHANGELOG.md`**: Update the relevant dated changelog entry instead.
- **Metadata comments scope**: Keep `<!-- Updated: ... -->` comments for instruction/docs files (for example `CLAUDE.md`, `.claude/rules/*`, `docs/CODEBASE_MAP.md`), not for `CHANGELOG.md`.
