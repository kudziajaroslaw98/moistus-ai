...
**Onboarding Persistence**: Persist onboarding state under a user-scoped storage key (`${ONBOARDING_STORAGE_KEY}:${currentUser.id}`) and wrap storage reads/writes in `try/catch` so blocked storage does not crash the slice. Hydrate that user-scoped state inside onboarding event handlers before branching on skip/complete flags. When pausing the controls coachmark tour, preserve `onboardingCoachmarkStep` + target context across coachmark and checklist minimize paths; minimize-pill body resume should expand back to checklist, while `startOnboardingTask('know-controls')` resumes coachmarks at the saved step (clamped to the active viewport sequence). Any checklist/pill CTA bound to this paused controls flow should read `Continue` (not `Start`).
