-- Moistus AI Collaboration Features - Database Setup Script
-- Run this script to set up all collaboration tables and policies
...
DROP TABLE IF EXISTS node_selections CASCADE;
DROP TABLE IF EXISTS mind_map_activities CASCADE;
DROP TABLE IF EXISTS user_presence CASCADE;
DROP TABLE IF EXISTS mind_map_shares CASCADE;
...
CREATE POLICY "Users can insert activities for maps they have access to" ON mind_map_activities
    FOR INSERT WITH CHECK (
        ...
        map_id IN (
            ...
            OR id IN (
                SELECT map_id FROM mind_map_shares
                WHERE user_id = (select auth.uid()) AND can_edit = true
            )
        )
    );
...
-- Setup complete
SELECT 'Moistus AI Collaboration tables setup completed successfully!' as setup_status;
