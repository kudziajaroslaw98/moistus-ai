...
-- Drop existing sharing tables if they exist (for clean setup)
...
COMMENT ON COLUMN share_tokens.token IS '6-character alphanumeric room code for easy sharing';
COMMENT ON COLUMN share_tokens.permissions IS 'JSON object defining access permissions for this share';
COMMENT ON COLUMN share_tokens.max_users IS 'Maximum number of concurrent users allowed';
COMMENT ON COLUMN share_tokens.current_users IS 'Current number of active users (updated by trigger)';

COMMENT ON COLUMN guest_users.session_id IS 'Unique session identifier for guest user tracking';
COMMENT ON COLUMN guest_users.fingerprint_hash IS 'Browser fingerprint for user identification across sessions';
COMMENT ON COLUMN guest_users.conversion_date IS 'Timestamp when guest converted to registered user';

COMMENT ON FUNCTION generate_room_code() IS 'Generates unique 6-character room codes avoiding confusing characters';
COMMENT ON FUNCTION validate_share_access(varchar, uuid, varchar) IS 'Validates share token access and returns permissions';
COMMENT ON FUNCTION cleanup_expired_shares() IS 'Cleans up expired shares and inactive guest users';

-- Setup complete
SELECT 'Moistus AI Sharing system setup completed successfully!' as setup_status;